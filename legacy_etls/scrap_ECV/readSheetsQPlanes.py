...
class readSheetsQPlanes:   
    def leer_datos_q_planes(self):
        df = []
        ...
        result = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range='Q_Planes_ECV!A:F').execute()
        # Extrae los valores del resultado
        values = result.get('values', [])[1:]
        
        # Crea el DataFrame df1
        df = pd.DataFrame(values, columns=['año', 'semestre', 'quintil', 'ingreso_del_quintil', 'percibe_plan', 'no_percibe_plan'])
    ...
    def transformar_tipo_datos(self, df):
        ...
        columnas_numericas = ['percibe_plan', 'no_percibe_plan']
        ...
        df['ingreso_del_quintil'] = df['ingreso_del_quintil'].astype(str)
