...
class readFileSalario:
    ...
    def read_file(self):
        ...
        for provincia_folder in os.listdir(base_dir):
            path_file = os.path.join(base_dir, provincia_folder, 'salario_promedio_construccion.xls')
            ...
            try:
                df = pd.read_excel(path_file, skiprows=4, decimal=',', thousands='.')
                df = df.dropna(subset=[df.columns[0]])

                df_prov = pd.DataFrame()
                df_prov['fecha'] = pd.to_datetime(df.iloc[:, 0], errors='coerce').dt.date
                df_prov['salario'] = pd.to_numeric(df.iloc[:, 1], errors='coerce')

                # ⛳ Detectar si la provincia es Buenos Aires
                ...
                df_prov['porcentaje_var_mensual'] = pd.to_numeric(df.iloc[:, col_mensual], errors='coerce') / 100
                df_prov['porcentaje_var_interanual'] = pd.to_numeric(df.iloc[:, col_interanual], errors='coerce') / 100
                df_prov['porcentaje_var_acumulada'] = pd.to_numeric(df.iloc[:, col_acumulada], errors='coerce') / 100
                ...
                if not self.total_pais_cargado and df.shape[1] >= 11:
                    ...
                    df_total['salario'] = pd.to_numeric(df.iloc[:, 4], errors='coerce')
                    df_total['porcentaje_var_mensual'] = pd.to_numeric(df.iloc[:, 8], errors='coerce') / 100
                    df_total['porcentaje_var_interanual'] = pd.to_numeric(df.iloc[:, 9], errors='coerce') / 100
                    df_total['porcentaje_var_acumulada'] = pd.to_numeric(df.iloc[:, 10], errors='coerce') / 100
        ...
        df_final = df_final[['fecha', 'id_provincia', 'salario',
