...
class Usuario(Base, UserMixin):
    ...
    # RBAC: Centro de Salud se vincula por CUIE SUMAR o SISA, Maternidad por ID
    cuie_code      = Column(String(50), ForeignKey('efectores_sisa.cuie'), nullable=True)
...
class Efector(Base):
    ...
    Contiene tanto el código SISA (oficial nacional) como el CUIE (Programa SUMAR).
    El campo 'cuie' es el usado como FK en el sistema; 'siisa' es el código SISA.
    """
    __tablename__ = 'efectores_sisa'

    id          = Column(Integer, primary_key=True, autoincrement=True)
    cuie        = Column(String(50), unique=True, index=True)   # Código SUMAR
    siisa       = Column(String(50), nullable=True, index=True) # Código SISA oficial
...
class Maternidad(Base):
    ...
    sisa_id            = Column(String(50), nullable=True)  # Código SISA de la maternidad
