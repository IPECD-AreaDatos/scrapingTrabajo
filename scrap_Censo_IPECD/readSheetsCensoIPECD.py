...
class readSheetsCensoIPECD:
    ...
    def transformar_tipo_datos(self, df):
        ...
        for columna in columnas_numericas:
            ...
            df.loc[no_nulos, columna] = df.loc[no_nulos, columna].replace({'%': '', ',': '.', r'[^\d.]':''}, regex=True).apply(pd.to_numeric)
...
df = readSheetsCensoIPECD().leer_datos_censo()
print(df)
