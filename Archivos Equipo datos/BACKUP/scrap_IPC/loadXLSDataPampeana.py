...
class LoadXLSDataPampeana:
    def loadInDataBase(self, file_path, host, user, password, database):
        ...
        try:
            ...
            # Obtener las fechas existentes en la tabla ipc_regionpampeana
            select_dates_query = "SELECT Fecha FROM ipc_regionpampeana"
            ...
            nivel_general = [cell.value for cell in sheet[69]][1:]
            alimento_y_bebidas_no_alcoholicas = [cell.value for cell in sheet[70]][1:]
            bebidas_alcoholicas_y_tabaco = [cell.value for cell in sheet[71]][1:]
            prendasVestir_y_calzado = [cell.value for cell in sheet[72]][1:]
            vivienda_agua_electricidad_gas_y_otros_combustibles = [cell.value for cell in sheet[73]][1:]
            equipamiento_y_mantenimiento_del_hogar = [cell.value for cell in sheet[74]][1:]
            salud = [cell.value for cell in sheet[75]][1:]
            transporte = [cell.value for cell in sheet[76]][1:]
            comunicación = [cell.value for cell in sheet[77]][1:]
            recreación_y_cultura = [cell.value for cell in sheet[78]][1:]
            educación = [cell.value for cell in sheet[79]][1:]
            restaurantes_y_hoteles = [cell.value for cell in sheet[80]][1:]
            bienes_y_servicios_varios = [cell.value for cell in sheet[81]][1:]
            ...
            #Agregamos REGION - PAMPEANA = 3
            
            update_query = "UPDATE ipc_regionpampeana SET id_region = 3;"
