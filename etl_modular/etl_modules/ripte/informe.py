...
class InformeRipte:
    def __init__(self, host, user, password, db):
        self.conn = ConexionBaseDatos(host, user, password, db)
        self.conn.connect_db()

        # Credenciales hardcodeadas
        self.usuario_gmail = "departamientoactualizaciondato@gmail.com"
        self.clave = "cmxddbshnjqfehka"

    def enviar_mensajes(self, nueva_fecha, nuevo_valor, valor_anterior):
        try:
            # 1. Obtener destinatarios desde la base
            #self.conn = ConexionBaseDatos(host, user, password, 'ipecd_economico')
            #self.conn.connect_db()
            ...
            #    FROM correos
            #"""
            #df_destinatarios = self.conn.read_sql(query)
            #destinatarios = df_destinatarios["correo"].tolist()

            email_receptores = ['manumarder@gmail.com']

            if not email_receptores:
                print("⚠️ No se encontraron destinatarios para RIPTE.")
                return
            ...
            print(f"✅ Informe RIPTE enviado a {len(email_receptores)} destinatarios.")

        except Exception as e:
            print(f"❌ Error al generar o enviar el informe RIPTE: {e}")
    ...
    def dolarizar(self, valor, fecha):
        """
        Convierte un valor en pesos a dólares usando el dólar blue promedio del mes de la fecha dada.
        """
        
        fecha_formateada = fecha.strftime('%Y-%m')
        fechaa = date(2025, 7, 29)

        query = f"""
            SELECT venta
            FROM dolar_blue
            WHERE DATE_FORMAT(fecha, '%%Y-%%m') = '{fechaa}'
            ORDER BY fecha DESC
            LIMIT 1;
        """

        resultado = pd.read_sql(query, self.conn.engine)
        if not resultado.empty:
            valor_dolar = resultado.iloc[0, 0]
            valor_dolarizado = round(valor / valor_dolar, 2)
            return valor_dolarizado
        else:
            raise ValueError(f"No se encontró valor del dólar blue para el mes: {fecha_formateada}")
    #Objetivo 
