...
class ripte_cargaUltimoDato:
    ...
    #Cargamos los datos
    def loadInDataBaseDatalakeEconomico(self):  
        table_name='ripte'
    ...
    def loadInDataBaseIPECD_Economico(self):  
        table_name='ripte'
        # Se toma el tiempo de comienzo
        start_time = time.time()
        tolerancia = 1.99         
       ...
       #Buscamos la tabla que contiene los datos
        ...
        # Obtener la última fecha y el último valor de ripte de la base de datos
        cursor = self.conn.cursor()
        cursor.execute(f"SELECT Fecha, ripte FROM {table_name} ORDER BY Fecha DESC LIMIT 1")
        ultima_fecha, ultimo_ripte = cursor.fetchone()
        ...
        if abs(contenido_float - ultimo_ripte) < tolerancia:
            ...
            # Sentencia SQL para insertar los datos en la tabla ripte
            insert_query = f"INSERT INTO {table_name} (Fecha, ripte) VALUES (%s, %s)"
