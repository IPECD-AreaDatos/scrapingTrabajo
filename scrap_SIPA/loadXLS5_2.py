...
class LoadXLS5_2:
    def loadInDataBase(self, file_path, host, user, password, database):
        ...
        try:
            ...
            table_name = "sipa_provincia_sin_estacionalidad"
            
            # Obtener las fechas existentes en la tabla 
            select_dates_query = "SELECT Fecha FROM sipa_provincia_sin_estacionalidad"
            cursor.execute(select_dates_query)
            existing_dates = [row[0] for row in cursor.fetchall()]


            # Leer el archivo Excel en un DataFrame de pandas
            df = pd.read_excel(file_path, sheet_name=14, skiprows=1)  # Leer el archivo XLSX y crear el DataFrame
            ...
            df = df.iloc[:-7]#Elimina las ultimas 6 filas siempre
            ...
            print("-----------------------------------------------")
            print("Se guardaron los datos de SIPA PROVINCIAL SIN ESTACIONALIDAD")
