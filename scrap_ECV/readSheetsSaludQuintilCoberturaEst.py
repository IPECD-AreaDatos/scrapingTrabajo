...
class readSheetsSaludQuintilCoberturaEst:
    def leer_datos_salud_quintil_cobertura_est(self):
        ...
        result = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range='Salud_Q_Cob_Est!A:K').execute()
        # Extrae los valores del resultado
        values = result.get('values', [])[1:]
        
        # Crea el DataFrame df1
        df = pd.DataFrame(values, columns=['Aglomerado', 'Año', 'Fecha', 'Semestre', 'Estado del dato', 'Quintil', 'Obra social/Prepaga', 'Planes y seguros', 'Sin Cobertura', 'Establecimiento_Privado', 'Establecimiento_Publico'])
        print(df)
