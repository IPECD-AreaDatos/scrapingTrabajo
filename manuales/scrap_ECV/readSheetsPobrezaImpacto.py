...
class readSheetsPobrezaImpacto:   
    def leer_datos_pobreza_impacto(self):
        df = []
        ...
        result = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range='Pobreza_Impacto_ECV!A:H').execute()
        # Extrae los valores del resultado
        values = result.get('values', [])[1:]
        
        # Crea el DataFrame df1
        df = pd.DataFrame(values, columns=['año', 'semestre', 'pobreza_eph', 'pobreza_ecv', 'pobreza_impacto_eph', 'indigencia_eph', 'indigencia_ecv', 'indigencia_impacto_eph'])
    ...
    def transformar_tipo_datos(self, df):
        ...
        columnas_numericas = ['pobreza_eph', 'pobreza_ecv', 'pobreza_impacto_eph', 'indigencia_eph', 'indigencia_ecv', 'indigencia_impacto_eph']
